...
def lookup_menu_item(
    ...
    logger.warning(
        "No exact match for '{}', suggestions: {}", item_name, suggestions[:3]
    )
    return {
        "found": False,
        "requested": item_name,
...
def add_item_to_order(
    ...
    """
    logger.info(
        "Adding item to order: {} (id={}, qty={})", item_name, item_id, quantity
    )
    ...
    for mod in modifiers or []:
        if mod.get("modifier_id") not in valid_modifier_ids:
            logger.warning(
                "Invalid modifier '{}' for item {}", mod.get("name"), item_name
            )
            return {
                "added": False,
    ...
    logger.info(
        "Item added successfully: {}x {} (size={})", quantity, item_name, resolved_size
    )
    return {
        "added": True,
        "item_id": item_id,
