...
Looking back, the core architecture held up well. But a few things I'd reconsider:
...
- **String-based prompt compilation** (`str.replace`) works for 3 variables but won't age well. Jinja2 or Langfuse's native compilation would be cleaner.
- **No item removal.** Customers say "actually, remove the hash browns" and right now there's no `remove_item` tool. Easy to add given the pure-function architecture, but it's a gap.
