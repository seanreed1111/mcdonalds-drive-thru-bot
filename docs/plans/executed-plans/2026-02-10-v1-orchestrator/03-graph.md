...
- [x] File exists: `src/orchestrator/orchestrator/graph.py`
- [x] Python can import the graph (no API key needed): `uv run --package orchestrator python -c "from orchestrator.graph import graph, DriveThruState; print('OK')"`
- [x] Graph has expected nodes: `uv run --package orchestrator python -c "from orchestrator.graph import graph; print(graph.get_graph().nodes)"`
- [x] Ruff passes: `uv run ruff check src/orchestrator/orchestrator/graph.py`
- [x] `langgraph.json` graph path resolves: the `graph` variable at module level is a compiled `CompiledStateGraph`
